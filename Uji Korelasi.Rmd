...
```{r}
data_contoh <- data.frame(x = c(1, 2, 3),
                   y = c(4, 5, 6),
                   z = c(7, 8, 9))
data_contoh
```

Data di atas adalah data yang akan kita hitung korelasinya.
...
```{r}
cor_pearson <- cor(data_contoh, method = "pearson")
cor_pearson
```
```{r}
library(corrplot)
plot.new()
corrplot(cor_pearson, method = "circle", type = "full", insig = "pch", )
title(main = "corpot dengan pearson method")
```
...
```{r}
cor_Spearman <- cor(data_contoh, method = "spearman")
cor_Spearman
```
